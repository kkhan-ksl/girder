...
1. To extend or override some of Girder's default style rules, place an ``.eslintrc.json`` file
   in a directory with or above the target Javascript files.
2. To completely override all of Girder's default style rules (i.e. disabling inheritance), add root
   ``"root": true`` to an ``.eslintrc.json``.
3. To natively utilize Girder's default style rules (from
...
Finally, plugin developers may add ``.eslintignore`` files,
`which also cascade<https://eslint.org/docs/user-guide/configuring#ignoring-files-and-directories>`_,
to prevent the static analysis of some files.
